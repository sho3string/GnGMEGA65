...
function split($inFile,$outPrefix, [Int32] $bufSize){
  ...
  while( $bytesRead = $stream.Read($barr,0,$bufsize)){
	...
	cls
	Write-Output " .------------------------------."
	Write-Output " |Building Ghosts'n Goblins ROMs|"
	Write-Output " '------------------------------'"
	...
	cmd /c copy /b gg3.bin+gg4.bin+gg5.bin $WorkingDirectory"/arcade/gng/rom1.bin"
	...
	cmd /c copy /b gg17.bin+gg16.bin+gg15.bin+gg15.bin $WorkingDirectory"/arcade/gng/spr1.bin"
	cmd /c copy /b gg14.bin+gg13.bin+gg12.bin+gg12.bin $WorkingDirectory"/arcade/gng/spr2.bin"
